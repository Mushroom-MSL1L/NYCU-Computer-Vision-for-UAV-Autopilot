...
while (1) :
    ...
    if ids is not None and has_foward == 1 :
            ...
            for i in range(len(ids)):
                ...
                if ids[i] == 1 and task_index == 0 :
                    ...
                    if z >= 40 : 
                        ...
                        for ii in range (5) :
                            drone.send_rc_control(0, 30, -10, 0)
                            time.sleep(0.5)
                ...
                ## midtem parts
                    ...
                    if z >= 40 : 
                        ...
                        for ii in range (7) :
                            ...
                            time.sleep(0.5)
                        drone.send_rc_control(0, 0, 0, 0)
                elif ids[i] == 2 and task_index == 3 and not_find_id4 : # follow people
                    x_update = MAX(x_pid.update(x_update, sleep=0), max_speed)
                    y_update = MAX(y_pid.update(y_update, sleep=0), max_speed)
                    z_update = MAX(z_pid.update(z_update, sleep=0), max_speed)
                    yaw_update = MAX(yaw_pid.update(yaw_update, sleep=0), max_speed)
        ...
        ### end of try catch 
            ...
            pass 
    ### end of ids is none 
    elif drone.is_flying and has_foward == 0 and task_index == 0 :
        for ii in range (2) :
            drone.send_rc_control(0, 45, 27, 0)
            time.sleep(1)
        has_foward = 1
